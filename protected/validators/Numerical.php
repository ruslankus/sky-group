 ...
 * @since 1.0
 */
class Numerical extends CValidator
{
	/**
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(!is_numeric($value))
		{
			...
			$message=$this->message!==null?$this->message:Yii::t('yii.skygroup','Must be a number.');
		...
		if($this->integerOnly)
		{
			...
			{
				$message=$this->message!==null?$this->message:Yii::t('yii.skygroup','Must be an integer.');
				$this->addError($object,$attribute,$message);
			}
		}
		else
		...
		{
			$message=$this->tooSmall!==null?$this->tooSmall:Yii::t('yii.skygroup','Is too small (minimum is {min}).');
			$this->addError($object,$attribute,$message,array('{min}'=>$this->min));
		}
		if($this->max!==null && $value>$this->max)
		{
			$message=$this->tooBig!==null?$this->tooBig:Yii::t('yii.skygroup','Is too big (maximum is {max}).');
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		if(($message=$this->message)===null)
			$message=$this->integerOnly ? Yii::t('yii.skygroup','Must be an integer.') : Yii::t('yii','{attribute} must be a number.');
		$message=strtr($message, array(
			'{attribute}'=>$label,
		));

		if(($tooBig=$this->tooBig)===null)
			$tooBig=Yii::t('yii.skygroup','Is too big (maximum is {max}).');
		...
		if(($tooSmall=$this->tooSmall)===null)
			$tooSmall=Yii::t('yii.skygroup','Is too small (minimum is {min}).');
